...
namespace Novels.ContentAddressing
{
    ...
    {
        private readonly Func<string, string> _resolveArtAddress;

        public ContentAddresses(
            string contentId,
            Func<string, string> resolveArtAddress = null)
        {
            if (string.IsNullOrWhiteSpace(contentId))
                throw new ArgumentException("Content ID must not be empty.", nameof(contentId));
            ContentId = contentId;
            _resolveArtAddress = resolveArtAddress;
        }
        ...
        public string LocationImage(string assetName) =>
            ResolveArt(ContentAddressConvention.LocationImage(ContentId, assetName));

        public string CharacterPrefab(string assetName) =>
            ContentAddressConvention.CharacterPrefab(ContentId, assetName);

        public string WardrobePrefab(string assetName) =>
            ContentAddressConvention.WardrobePrefab(ContentId, assetName);
        ...
        public string CharacterMainBody(string name, string view, string candidate) =>
            ResolveArt(ContentAddressConvention.CharacterMainBody(
        ...
        private string ResolveArt(string address) =>
            _resolveArtAddress == null || string.IsNullOrEmpty(address)
                ? address
                : _resolveArtAddress(address);
