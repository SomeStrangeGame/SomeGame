...
namespace Ink.UnityIntegration {
	...
	public class InkCompiler : ScriptableObject {
		...
		static int compileProgressID = -1;
		#endif
		...
		// A domain reload terminates the worker thread but ScriptableSingleton keeps
		// compilationStack in Library/InkCompiler.asset. Treating the persisted
		// Compiling item as live produces an immediate false timeout because
		// DateTime and the static Progress id do not survive the reload.
		private static void RecoverInterruptedCompilation() {
			compileThreadActive = false;
			bool recovered = false;
			foreach(var item in instance.compilationStack) {
				if(item.state != CompilationStackItemState.Compiling) continue;
				item.state = CompilationStackItemState.Queued;
				item.compiledJson = null;
				item.logOutput.Clear();
				item.unhandledErrorOutput.Clear();
				item.startTime = default(DateTime);
				item.endTime = default(DateTime);
				recovered = true;
			}
			...
			if(!recovered) return;
			instance.Save(true);
			EditorApplication.delayCall += TryCompileNextFileInStack;
		}
		...
		private static void TryCompileNextFileInStack () {
			...
			// Starting a worker while Unity is compiling scripts allows a domain reload
			// to tear it down. Update() retries the queued item once compilation ends.
		...
		private static void BeginCompilingFile(CompilationStackItem item) {
			...
			#if UNITY_2020_2_OR_NEWER
			if(compileProgressID < 0) OnBeginCompilationStack();
		...
		private static void OnCompleteCompilationStack () {
			...
			#if UNITY_2020_2_OR_NEWER
			if(compileProgressID >= 0) Progress.Remove(compileProgressID);
			compileProgressID = -1;
			#endif
