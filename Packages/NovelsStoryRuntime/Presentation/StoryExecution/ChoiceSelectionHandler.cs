...
namespace Novels.StoryExecution
{
    internal sealed class ChoiceSelectionHandler
    {
        ...
        private async UniTask Commit(int? choiceId)
        {
            try
            {
                if (_request.Services.FlushCheckpoint != null)
                    await _request.Services.FlushCheckpoint();
                if (choiceId.HasValue)
                {
                    _request.Services.Story.SetChoice(choiceId.Value);
                    _request.Services.OnChoiceSelected?.Invoke(choiceId.Value);
                }
                _request.Completed.TrySetResult();
            }
            catch (Exception exception)
            {
                // Stop this queue instead of presenting an unsaved next dialogue.
                _request.Completed.TrySetException(exception);
            }
        }
