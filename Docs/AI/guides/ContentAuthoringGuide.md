...
Эпизод после служебных metadata должен сразу переходить к первой настоящей
сцене. Не добавляйте одно-вариантный стартовый выбор `Начать`, `Начать историю`
или `Играть`: запуск истории уже является согласием начать эпизод. Первый
показанный выбор должен выражать реальное решение игрока.
