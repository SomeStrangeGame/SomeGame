...
- `publish-current-main`: publication prepared in local commits `fce4ea25`, `0e0fbb2a`, `6cb5eb35`; origin/base matched and all content gates passed. User-approved cleanup removed only post-validation serialization noise from uncommitted `Projects/**`. Fresh Novels editor-gate timed out waiting for Pipeline port; previous per-scope compile evidence remains recorded. Pending: docs commit, non-force push/SHA parity and branch creation.
