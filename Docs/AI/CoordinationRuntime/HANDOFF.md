...
- `publish-all-local-main`: весь текущий dirty tree собран для общей публикации. `git diff --check`, automation tests, editor content builds для catalog и всех 12 stories, а также fresh Novels Editor compile passed; EditMode test assemblies отсутствуют. После одного штатного licensing recovery-loop (TERM только конфликтующего PID 57057; license/cache/socket не удалялись) Unity gates стабильны. Pending: атомарные commits, merge четырёх remote-коммитов без force и финальная сверка SHA; существующие manual visual gates остаются в handoff.
