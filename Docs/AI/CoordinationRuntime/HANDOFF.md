...
- `toybedtime-rollback`: неудачный story-local Bubble variant полностью удалён, `_authoringChunks: []` восстановлен. Fresh editor release `a5276e25…` содержит definition и все три исходных фона, local Bubble отсутствует; validate/build и four-root-assets bundle audit passed, scoped story diff clean. Pending: ручной вход в историю; новый детский Bubble проектировать заново только через Unity-authored prefab и отдельный visual gate.
