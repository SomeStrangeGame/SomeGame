...
- `toybedtime-choice-icons`: shared fallback восстановлен; broken raw-YAML Bubble variant отсутствует. Ink tags и Sprite import подтверждены, release `a9c3bae3...` содержит обе choice sprites. После portrait evidence жёлтый child-background, создаваемый helper, удалён: теперь перекрашивается исходный `Button.targetGraphic`, icon и text явно подняты над ним, обычный fallback path не меняется. Live Novels Editor auto-compile ready, console compiler errors empty, diff-check passed. Pending: portrait replay первого choice; Bubble variant создавать только через Unity-authored serialization.
