...
2. **Стандартная** — один раз после завершения логического блока: быстрая
   проверка плюс один bounded compile и только относящиеся к изменению tests.
   Несколько связанных мелких правок объединяются в один validation slot.
3. **Релизная** — только по прямому запросу разработчика либо перед реальным
   выпуском: стандартная проверка плюс необходимые content build, Player/APK,
   device smoke и visual gates.

Changed-path plan может понизить объём внутри выбранного уровня, но не должен
самовольно запускать более дорогой уровень. Если задача требует более сильного
доказательства для безопасности контракта, агент до запуска явно сообщает
причину и новый уровень. APK, эмулятор и визуальный smoke не являются обычной
частью промежуточной разработки.
