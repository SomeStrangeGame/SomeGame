...
- Каждая задача может владеть своим Unity Editor и Android emulator. Изоляция
  обязательна по точному project/worktree path, PID, AVD/serial и output paths;
  совпадающий mutable target остаётся эксклюзивным ресурсом.
