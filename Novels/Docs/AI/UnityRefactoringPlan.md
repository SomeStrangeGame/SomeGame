...
4. Ink authoritatively declares audio, background, and speaker references. The common Editor index resolves declared string variables, derives delivery dependencies from the story, and rejects resource references that cannot be resolved statically; no parallel dependency lists are stored in `NovelContentAsset`.
   `Bundles.MediaFileConvention` fixes extensionless audio to `.wav` and video to `.mp4`. Audio with a non-default format includes its extension directly in Ink; validation reports format mismatches and ambiguity instead of relying on serialized extension settings.
...
6. Cancellation-aware fade and movement live in the reusable `somegame.ui-transitions` package. Bubble, Character, Location, and Notification views use it without changing prefab dimensions or serialized timing/layout values.
